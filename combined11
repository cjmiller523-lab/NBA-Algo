...
def parse_props(pid, stat, odds_obj):
    ...
    clean = [x for x in out if int(x["odds"]) >= -600]
    return clean

# =====================================
# SLEEPER INJURIES ONLY
# =====================================
...
def process_all_games():
    ...
    for game in games:
        ...
        def process_team(team_p, ids, odds_obj, tname):
            for pid in ids:
                ...
                for stat in STATS:
                    ...
                    # --- NEW LOGIC: only 1 prop per threshold
                    best = {}  # thr → row
                    ...
                    for prop in props:
                        ...
                        if not prev:
                            ...
                            if (
                                row["confidence"] > prev["confidence"] or
                                (row["confidence"] == prev["confidence"] and row["edge"] > prev["edge"]) or
                                (row["confidence"] == prev["confidence"] and row["edge"] == prev["edge"] and row["hit_rate"] > prev["hit_rate"])
                            ):
                                best[thr] = row
        ...
        if rows:
            rows = sorted(rows, key=lambda r: (-r["confidence"], -r["edge"], -r["hit_rate"]))[:10]

            print("\nTop 10 Props:")
            for r in rows:
                print(
                    f"{r['player']} ({r['team']}) - {r['stat']} line {r['line']} @ {r['odds']} | "
                    f"Hit {r['hits']}/{r['total']} ({r['hit_rate']}%) | Conf {r['confidence']} | Edge {r['edge']}"
                )

            results.append({"game_label": label, "rows": rows})
...
# TWITTER EXPORT
# =====================================
def build_twitter_posts(results):
    if not results:
        return

    os.makedirs("twitter_posts", exist_ok=True)
    date_str = datetime.now().strftime("%Y-%m-%d")
    filepath = os.path.join("twitter_posts", f"{date_str}.txt")

    lines = []

    for g in results:
        lines.append(f"🏀 {g['game_label']} — Top Props\n")

        rows = g["rows"][:5]  # top 5 only

        for i, r in enumerate(rows, 1):
            pct = r["hit_rate"]
            lines.append(
                f"{i}) {r['player']} – {r['line']} {r['stat'].upper()} ({r['odds']})\n"
                f"   Hit: {r['hits']}/{r['total']} ({pct}%)\n"
            )
...
def build_pdf_report(results):
    ...
    story.append(Paragraph(f"NBA Props Confidence Report – {today}", styles["Title"]))
    ...
    for g in results:
        ...
        for r in g["rows"]:
            data.append([
                r["player"], r["team"], r["stat"], str(r["line"]),
                str(r["odds"]), f"{r['threshold']}+",
                f"{r['hits']}/{r['total']}", f"{r['hit_rate']}%",
                str(r["confidence"]), str(r["edge"])
            ])
